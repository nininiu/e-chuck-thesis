...
已有的静电力测量方法均具有较大的不确定性。为了得到更准确、可信的测量结果，需通过对检测原理的分析，解决一些已有方案中的共有问题，并尽可能减小乃至消除测量中的不确定性，设计出改进检测方案。由于除背吹平衡法以外的方案均在原理上就具有较大不确定度（见\ref{sec:priorArt}节），本章以背吹平衡法作为基础来设计新方案。
